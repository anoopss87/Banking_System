...
int main(int argc, char** argv){
    ...
    for(int j=0;j<NUM_OF_SERVERS;j++)
    {
        char key[2];
        sprintf(key, "s%d", j+1);
        serverList[key] = "anoop-VPCEB46FX";
        serverPortList[key] = 8000+j;
    }
        ...
        if(m_pkt.master_server == 1)
        {
            ...
            if(!flag)
            {
                ...
                *p_int = 1;
                ...
                printf("=======================================\n");
                printf("Family %d\n", my_addr[0].sin_family);
                printf("Port %d\n", my_addr[0].sin_port);
                printf("Address %x\n", my_addr[0].sin_addr.s_addr);
                printf("Zero %s\n", my_addr[0].sin_zero);

                printf("Family %d\n", my_addr[1].sin_family);
                printf("Port %d\n", my_addr[1].sin_port);
                printf("Address %x\n", my_addr[1].sin_addr.s_addr);
                printf("Zero %s\n", my_addr[1].sin_zero);
